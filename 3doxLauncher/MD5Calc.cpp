...
#define	FETCH_32(p)						\
	...
	(((UINT32)*((const UINT8 *)(p) + 1)) << 8) |	\
	(((UINT32)*((const UINT8 *)(p) + 2)) << 16) |	\
	(((UINT32)*((const UINT8 *)(p) + 3)) << 24))
 ...
 */
static void
Encode(unsigned char *output, UINT32 *input, unsigned int len)
{
	unsigned int i, j;
...
static void MD5Transform(UINT32, UINT32, UINT32, UINT32,
    const UINT8 [64], MD5_CTX *);
 ...
 */
static void
MD5Transform(UINT32 a, UINT32 b, UINT32 c, UINT32 d,
    const UINT8 block[64], MD5_CTX *context)
{
	/* Register (instead of array) is a win in most cases */
	register UINT32 x0, x1, x2, x3, x4, x5, x6, x7;
	register UINT32 x8, x9, x10, x11, x12, x13, x14, x15;
