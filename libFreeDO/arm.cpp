...
//--------------------------äëÿ óñëîâíîãî ïîëÿ êîìàíäû-------------------------------------------
    //flags - N Z C V  -  31...28
const static uint16 cond_flags_cross[]={   //((cond_flags_cross[cond_feald]>>flags)&1)  -- ïðèìåð ïðîâåðêè
...
{
//ïàìÿòü ïðèñòàâêè - èìååò ñìûñë ñäåëàòü åå âûäåëÿìîé------------------------
    uint8 *Ram;//[RAMSIZE];
    uint8 *Rom;//[ROMSIZE*2];
	uint8 *NVRam;//[NVRAMSIZE];

//îïðåäåëåíèå ðåãèñòðîâ ARM60
	uint32 USER[16];
	uint32 CASH[7];
    uint32 SVC[2];
    uint32 ABT[2];
	...
	//bool nFIQ; //âíåøíåå ïðåðûâàíèå, óñòàíàâëèâàåòñÿ äðóãèìè ïðîöåññîðàìè
	int SecondROM;	//ñåëåêòîð ÏÇÓ (ðóì è øðèôò)
	//bool MAS_Access_Exept;	//çàâåäåíî äëÿ èñêëþ÷åíèé äîñòóïà ê ïàìÿòè
};
...
static int CYCLES;	//çäåñü ñ÷èòàåì öèêëû
...
//#define gFIQ	arm.nFIQ //âíåøíåå ïðåðûâàíèå, óñòàíàâëèâàåòñÿ äðóãèìè ïðîöåññîðàìè
#define gSecondROM	arm.SecondROM	//ñåëåêòîð ÏÇÓ (ðóì è øðèôò)
...
{
        XMemCpy(buff,&arm,sizeof(ARM_CoreState));
        XMemCpy(((uint8*)buff)+sizeof(ARM_CoreState),pRam,RAMSIZE);
        XMemCpy(((uint8*)buff)+sizeof(ARM_CoreState)+RAMSIZE,pRom,ROMSIZE*2);
        XMemCpy(((uint8*)buff)+sizeof(ARM_CoreState)+RAMSIZE+ROMSIZE*2,pNVRam,NVRAMSIZE);
}
...
void _arm_Load(void *buff)
{
 ...
 uint8 *tRom=pRom;
 uint8 *tNVRam=pNVRam;
        XMemCpy(&arm,buff,sizeof(ARM_CoreState));
        XMemCpy(tRam,((uint8*)buff)+sizeof(ARM_CoreState),RAMSIZE);
        XMemCpy(tRom,((uint8*)buff)+sizeof(ARM_CoreState)+RAMSIZE,ROMSIZE*2);
        XMemCpy(tNVRam,((uint8*)buff)+sizeof(ARM_CoreState)+RAMSIZE+ROMSIZE*2,NVRAMSIZE);

        XMemCpy(tRam+3*1024*1024,tRam+2*1024*1024, 1024*1024);
        XMemCpy(tRam+4*1024*1024,tRam+2*1024*1024, 1024*1024);
        XMemCpy(tRam+5*1024*1024,tRam+2*1024*1024, 1024*1024);
        XMemCpy(tRam+6*1024*1024,tRam+2*1024*1024, 1024*1024);
        XMemCpy(tRam+7*1024*1024,tRam+2*1024*1024, 1024*1024);
        XMemCpy(tRam+8*1024*1024,tRam+2*1024*1024, 1024*1024);
        XMemCpy(tRam+9*1024*1024,tRam+2*1024*1024, 1024*1024);
        XMemCpy(tRam+10*1024*1024,tRam+2*1024*1024, 1024*1024);
        XMemCpy(tRam+11*1024*1024,tRam+2*1024*1024, 1024*1024);
        XMemCpy(tRam+12*1024*1024,tRam+2*1024*1024, 1024*1024);
        XMemCpy(tRam+13*1024*1024,tRam+2*1024*1024, 1024*1024);
        XMemCpy(tRam+14*1024*1024,tRam+2*1024*1024, 1024*1024);
        XMemCpy(tRam+15*1024*1024,tRam+2*1024*1024, 1024*1024);
        XMemCpy(tRam+16*1024*1024,tRam+2*1024*1024, 1024*1024);
        XMemCpy(tRam+17*1024*1024,tRam+2*1024*1024, 1024*1024);
...
void ARM_RestUserRONS()
{
   switch(arm_mode_table[(CPSR&0x1f)])
   {
     case ARM_MODE_USER:
             break;
     case ARM_MODE_FIQ:
             XMemCpy(RON_FIQ,&RON_USER[8],7<<2);
             XMemCpy(&RON_USER[8],RON_CASH,7<<2);
             break;
     case ARM_MODE_IRQ:
...
void ARM_RestFiqRONS()
{
   ...
   {
     case ARM_MODE_USER:
             XMemCpy(RON_CASH,&RON_USER[8],7<<2);
             XMemCpy(&RON_USER[8],RON_FIQ,7<<2);
             break;
     case ARM_MODE_FIQ:
             break;
     case ARM_MODE_IRQ:
             XMemCpy(RON_CASH,&RON_USER[8],5<<2);
			 RON_IRQ[0]=RON_USER[13];
			 RON_IRQ[1]=RON_USER[14];
             XMemCpy(&RON_USER[8],RON_FIQ,7<<2);
             break;
...
void ARM_RestIrqRONS()
{
   switch(arm_mode_table[(CPSR&0x1f)])
   {
     ...
     case ARM_MODE_FIQ:
             XMemCpy(RON_FIQ,&RON_USER[8],7<<2);
             XMemCpy(&RON_USER[8],RON_CASH,5<<2);
			 RON_USER[13]=RON_IRQ[0];
			 RON_USER[14]=RON_IRQ[1];
             break;
     case ARM_MODE_IRQ:
...
//	MAS_Access_Exept=false;
	...
	gSecondROM=0;
        ...
        // Allocate physical memory with 64-KB pages
         pRam = (uint8*)XPhysicalAlloc(RAMSIZE+1024*1024*16, MAXULONG_PTR, 0,
    ...
    XMemSet( pRam, 0, RAMSIZE+1024*1024*16);
    XMemSet( pRom, 0, ROMSIZE*2);
    XMemSet( pNVRam,0, NVRAMSIZE);
//    gFIQ=false;
